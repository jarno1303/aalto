...
internal object AudioEffects {
    ...
    /**
     * Makes this station as loud as the user wants it. Quieter is done with
     * the player's own volume, louder with the loudness enhancer, because the
     * player cannot go above its own level.
     */
    fun applyStationGain(context: Context, stationId: String?) {
        val gainDb = AudioSettings.stationGainDb(context, stationId)
        runCatching { player?.volume = decibelsToVolume(gainDb) }
            .onFailure { Log.w(TAG, "volume not applied", it) }
        runCatching {
            loudness?.let { enhancer ->
                val boostMb = (gainDb.coerceAtLeast(0) * 100)
